 ...
 * pre-flight request.</li> <li>NOT_CORS: A normal same-origin request.</li>
 * <li>INVALID_CORS: A cross-origin request which is invalid.</li> </ul> </li>
 * <li><b>cors.request.headers:</b> Request headers sent as
 * 'Access-Control-Request-Headers' header, for pre-flight request.</li> </ul>
 * </p>
 *
...
public final class WebDavCompliantCorsFilter implements Filter {
    ...
    public WebDavCompliantCorsFilter() {
    ...
    public void doFilter(final ServletRequest servletRequest,
        ...
        WebDavCompliantCorsFilter.CORSRequestType requestType = checkRequestType(request);

        // Adds CORS specific attributes to request.
        if (decorateRequest) {
            WebDavCompliantCorsFilter.decorateCORSProperties(request, requestType);
        }
        switch (requestType) {
            case SIMPLE:
    ...
    protected void handleSimpleCORS(final HttpServletRequest request,
        ...
        WebDavCompliantCorsFilter.CORSRequestType requestType = checkRequestType(request);
        if (!(requestType == WebDavCompliantCorsFilter.CORSRequestType.SIMPLE
        ...
        final String origin = request
                .getHeader(WebDavCompliantCorsFilter.REQUEST_HEADER_ORIGIN);
        ...
        if ((exposedHeaders != null) && (exposedHeaders.size() > 0)) {
            ...
            response.addHeader(
                    WebDavCompliantCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_EXPOSE_HEADERS,
    ...
    protected void handlePreflightCORS(final HttpServletRequest request,
        ...
        String accessControlRequestMethod = request.getHeader(
                WebDavCompliantCorsFilter.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_METHOD);
        ...
        String accessControlRequestHeadersHeader = request.getHeader(
                WebDavCompliantCorsFilter.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_HEADERS);
        ...
        if (preflightMaxAge > 0) {
            response.addHeader(
                    WebDavCompliantCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_MAX_AGE,
                    String.valueOf(preflightMaxAge));
        }

        // Section 6.2.9
        response.addHeader(
                WebDavCompliantCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_METHODS,
                accessControlRequestMethod);

        // Section 6.2.10
        if ((allowedHttpHeaders != null) && (!allowedHttpHeaders.isEmpty())) {
            response.addHeader(
                    WebDavCompliantCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_HEADERS,
    ...
    private void handleInvalidCORS(final HttpServletRequest request,
            final HttpServletResponse response, final FilterChain filterChain) {
        String origin = request.getHeader(WebDavCompliantCorsFilter.REQUEST_HEADER_ORIGIN);
...
//        if (log.getLevel() == Level.FINEST) {
        ...
        switch (corsRequestType) {
            case SIMPLE:
                request.setAttribute(
                        WebDavCompliantCorsFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST,
                        Boolean.TRUE);
                request.setAttribute(WebDavCompliantCorsFilter.HTTP_REQUEST_ATTRIBUTE_ORIGIN,
                        request.getHeader(WebDavCompliantCorsFilter.REQUEST_HEADER_ORIGIN));
                request.setAttribute(
                        WebDavCompliantCorsFilter.HTTP_REQUEST_ATTRIBUTE_REQUEST_TYPE,
            ...
            case PRE_FLIGHT:
                ...
                request.setAttribute(
                        WebDavCompliantCorsFilter.HTTP_REQUEST_ATTRIBUTE_REQUEST_HEADERS, headers);
                break;
            case NOT_CORS:
                request.setAttribute(
                        WebDavCompliantCorsFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST,
     ...
     * @param allowedHttpMethods A {@link String} of comma separated HTTP
     * methods.
     * @param allowedHttpHeaders A {@link String} of comma separated HTTP
     * headers.
     * @param exposedHeaders A {@link String} of comma separated headers that
     * needs to be exposed.
     * @param supportsCredentials "true" if support credentials needs to be
     * enabled.
    ...
    /**
     * Checks if a given origin is valid or not. Criteria: <ul> <li>If an
     * encoded character is present in origin, it's not valid.</li> <li>Origin
     * should be a valid {@link URI}</li> </ul>
     *
     * @param origin
